...
from dana_codes.src.agent.deep import EpisodicACAgent
...
def train_model():
    for episode in tqdm(range(EPISODES)):
        ...
        cum_rewards.append(sum(agent.episode_pure_rewards))
        losses.append(loss)
        agent.reset_episode()
...
if __name__ == '__main__':
    ...
    layers = [f'fc:{state_shape[0]},64', 'relu']

    agent = EpisodicACAgent(lr=LR, gamma=GAMMA, layers=layers,
                     state_shape=state_shape, n_actions=n_actions)

    cum_rewards, steps, losses = [], [], []

    plot_folder_name = 'll_episodicAC_plots/'
    model_folder_name = 'll_episodicAC_models/'
